...
export class CrossChainMessageUtils {
  ...
  static generateMessageId(
    sender: string, // Address as hex string or Solana base58 string
    tokenId: string,
    ...
    try {
      // Try as ethereum address first
      senderBytes = CrossChainMessageUtils.ethereumAddressToBytes32(sender);
    } catch {
      // If that fails, try as Solana address
      try {
        const solPubkey = new PublicKey(sender);
        senderBytes = CrossChainMessageUtils.solanaPublicKeyToBytes32(solPubkey);
      } catch {
        // If both fail, treat as hex string
        senderBytes = ethers.getBytes(sender);
        if (senderBytes.length < 32) {
          const padded = new Uint8Array(32);
          padded.set(senderBytes, 32 - senderBytes.length);
          senderBytes = padded;
        }
      }
    }
  ...
  static decodeFromEVM(encodedData: string): NFTTransferMessage {
    try {
      ...
      const message = {
        ...
        originChain: Number(decoded[3]),
        destinationChain: Number(decoded[4]),
