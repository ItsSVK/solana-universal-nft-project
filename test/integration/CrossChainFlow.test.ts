...
describe('Universal NFT Protocol - Integration Tests', () => {
  ...
  before(async () => {
    ...
    charlieEth = new ethers.Wallet(
      process.env.PRIVATE_KEY_CHARLIE || '0x' + '03'.repeat(32),
      provider
    );
  ...
  describe('Full Cross-Chain Journey: ZetaChain → Base → Solana → ZetaChain', () => {
    ...
    it('Step 1: Mint NFT on ZetaChain', async function () {
      ...
      console.log(
        `   Origin Chain: ${CrossChainMessageUtils.getChainName(currentMessage.originChain)}`
      );
      console.log(
        `   Destination: ${CrossChainMessageUtils.getChainName(currentMessage.destinationChain)}`
      );
      console.log(
        `   Message ID: 0x${Buffer.from(currentMessage.messageId).toString('hex').slice(0, 16)}...`
      );
    ...
    it('Step 2: Receive NFT on Base Sepolia', async function () {
      ...
      expect(
        Buffer.from(decodedMessage.originContract).equals(
          Buffer.from(currentMessage.originContract)
        )
      ).to.be.true;

      console.log(`   Received Token ID: ${decodedMessage.tokenId}`);
      console.log(
        `   Preserved Origin: ${CrossChainMessageUtils.getChainName(decodedMessage.originChain)}`
      );
      console.log(`   Current Chain: Base Sepolia`);
      console.log(
        `   Recipient: ${CrossChainMessageUtils.bytes32ToEthereumAddress(decodedMessage.recipient)}`
      );
    ...
    it('Step 3: Transfer NFT from Base Sepolia to Solana', async function () {
      ...
      const transferMessage = MessageBridge.createEvmToSolanaMessage({
        ...
        originContractAddress: CrossChainMessageUtils.bytes32ToEthereumAddress(
          currentMessage.originContract
        ),
        nonce: '2',
      });
      ...
      console.log(
        `   Origin preserved: ${CrossChainMessageUtils.getChainName(currentMessage.originChain)}`
      );
    ...
    it('Step 5: Transfer NFT back to ZetaChain (completing the loop)', async function () {
      ...
      // Track return message to prevent replay
      const returnMessageIdHex = Buffer.from(currentMessage.messageId).toString('hex');
      expect(processedMessages.has(returnMessageIdHex)).to.be.false;
      processedMessages.add(returnMessageIdHex);
    ...
    it('Verify Complete Journey Integrity', async function () {
      ...
      routesUsed.forEach((route) => {
        ...
        console.log(
          `   ✓ Route supported: ${CrossChainMessageUtils.getChainName(route.from)} → ${CrossChainMessageUtils.getChainName(route.to)}`
        );
      });
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    it('should handle corrupted message data', async function () {
      ...
      // Create more severely corrupted data that will definitely cause decoding to fail
      const corrupted = '0x' + 'FF'.repeat(100); // Invalid ABI-encoded data
  ...
  describe('Performance and Scalability', () => {
    ...
    it('should efficiently encode/decode large metadata', async function () {
      ...
      const largeMetadata = {
        ...
        image:
          'https://example.com/very-long-image-url-that-might-be-used-in-production-environments.png',
